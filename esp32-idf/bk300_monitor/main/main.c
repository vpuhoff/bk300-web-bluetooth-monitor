...
void app_main(void) {
  ...
  // Поднимаем log-уровень для Bluedroid-тегов до DEBUG (CONFIG_LOG_MAXIMUM_LEVEL=DEBUG
  // обеспечивает, что код DEBUG-вызовов вообще скомпилирован). Видим HCI/GATT-трассы.
  ...
  ESP_LOGI(TAG, "BK300 monitor (ESP-IDF / Bluedroid) initialized.");
}
