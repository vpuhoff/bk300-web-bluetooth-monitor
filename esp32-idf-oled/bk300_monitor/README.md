...
кадра с напряжением (`0B4B`)** (после первого кадра обновляется
`s_voltage_ts_us` в OLED). До первого `0B4B` счётчик `rx` может не
показываться (`voltage_ts == 0`).

Тихая поломка в первой версии recovery: после **re-kick** обнулялся
отсчёт «тишины», и при цикле nudge/re-kick **никогда** не накапливались
75 с от *реального* последнего `0B4B`, поэтому `rx` снова мог уходить
в **200+ с** без скана. Сейчас `last_0b4b_us` привязан только к
распарсенным кадрам напряжения; при тишине **≥ ~75 с** вызывается
`gattc_close` **каждую секунду**, пока линк не отвалится, затем
`Reconnecting` и скан.

Дополнительно при **rx > ~30 с** (после хотя бы одного `0B4B`) шлётся
**GATT Read** по FFF1, иначе FFF2, иначе чтение CCCD: при ошибке,
отклонении стеком или **таймауте 5 с** без колбэка — `force_disconnect`
и полный коннект заново; если ответ **OK**, но с последнего `0B4B` уже
**≥ ~45 с** — тоже разрыв (на случай «ATT жив, телеметрия нет»).

Если `rx` растёт при устройстве рядом — смотри логи `W` (`0B4B stale …`,
`link probe …`, `force disconnect`).
