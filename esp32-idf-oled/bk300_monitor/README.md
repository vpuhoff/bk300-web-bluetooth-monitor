...
`esp32-idf/bk300_monitor`. Этот проект отличается **только** добавкой
`main/oled.[ch]`, `main/oled_font.h`, `main/Kconfig.projbuild` и
точечными вызовами `oled_set_status()` / `oled_set_voltage()` в `main.c`.
...
| Статус | Когда |
| --- | --- |
| `Init BLE` | в `app_main`, до `esp_bluedroid_enable` |
...
| `main/CMakeLists.txt` | + `oled.c` и зависимости (`driver`, `esp_driver_i2c`, `esp_timer`) |
| `main/main.c` | + точечные вызовы `oled_set_status()` (на каждой смене BLE-фазы) и `oled_set_voltage()` (в `bk300_drain_frames` при типе `0x0B4B`) |
...
I (xxx) bk300: BK300 OLED monitor (ESP-IDF / Bluedroid) initialized.
