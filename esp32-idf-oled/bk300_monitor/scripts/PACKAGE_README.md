# bk300_oled_monitor — переносной пакет прошивки

Этот архив содержит готовые бинари ESP-IDF проекта `bk300_oled_monitor`
(BK300 + SSD1306 OLED 128×64) и скрипты `flash.sh` / `flash.bat`, чтобы
прошить устройство с другого компа без полной установки ESP-IDF.

Имя проекта (`PROJECT_NAME`) и версия пакета (`PKG_VERSION`) лежат в
`flash.env` — flash-скрипты их сами читают.
