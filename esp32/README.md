# ESP32 (Arduino) — BK300 Serial / OLED monitor

В этом каталоге два независимых скетча:

| Скетч | Куда выводит напряжение | Когда брать |
| --- | --- | --- |
| `bk300_oneshot_sleep.ino` | Serial (115200) | базовая отладка, голая плата без экрана |
| `bk300_oled/bk300_oled.ino` | SSD1306 128×64 OLED (I²C) + Serial | железка с прикрученным OLED-модулем |

Оба используют **одну и ту же BLE-логику** (тот же протокол / kick /
Unix-epoch timestamp); OLED-вариант просто добавляет вывод. Если новый
скетч `bk300_oled` ведёт себя странно — сравни лог в Serial с базовым,
у них одинаковая последовательность.
